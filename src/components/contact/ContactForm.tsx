...
export function ContactForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (trimmedName.length < 2) {
      toast({ title: t("contact.toastError"), description: t("contact.validationName"), variant: "destructive" });
      return;
    }
    if (contactMethod === "phone" && phone.length !== 9) {
      toast({ title: t("contact.toastError"), description: t("contact.validationPhone"), variant: "destructive" });
      return;
    }
    if (contactMethod === "telegram" && !telegram.replace(/^@/, "").trim()) {
      toast({ title: t("contact.toastError"), description: t("contact.validationTelegram"), variant: "destructive" });
      return;
    }
    if (trimmedMessage.length < 5) {
      toast({ title: t("contact.toastError"), description: t("contact.validationMessage"), variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const pgErr = err as { message?: string; hint?: string; code?: string };
      const detail = isNetworkError(err)
        ? NETWORK_ERROR_MESSAGE_UZ
        : pgErr.message?.includes("rate_limit_exceeded") || pgErr.hint?.includes("kuting")
          ? t("contact.toastRateLimit")
          : t("contact.toastErrorDesc");

      toast({ title: t("contact.toastError"), description: detail, variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }
