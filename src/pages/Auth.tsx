...
const Auth = () => {
  ...
  const startCooldown = (seconds: number) => {
    // Clear any existing interval so timers never stack
  ...
  // Only allow same-origin paths ("/...") — blocks open redirects like
  // "//evil.com" or "https://evil.com" passed via location.state
  ...
  const handleGoogleLogin = async () => {
    ...
    if (googleError) {
      setError(isNetworkError(googleError) ? NETWORK_ERROR_MESSAGE_UZ : 'Google orqali kirish xatolik yuz berdi');
  ...
  return (
    ...
    <SEO
      ...
      description="Avtotestlar.uz hisobingizga kiring yoki yangi hisob oching."
      path="/auth"
      noIndex={true}
    />
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 flex flex-col items-center justify-center p-4">
      ...
      <Card className="w-full max-w-md p-6 md:p-8 bg-card border-border shadow-xl">
        ...
        <button onClick={() => navigate('/')} className="text-center mb-6 w-full cursor-pointer">
          ...
          <h1 className="text-2xl font-bold text-foreground">Avtotestlar.uz</h1>
        ...
        <form onSubmit={handleEmailLogin} className="space-y-4">
          ...
          {failCount >= 5 && (
            <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/30 flex items-center gap-2">
              <AlertCircle className="w-4 h-4 text-destructive flex-shrink-0" />
              <span className="text-sm text-destructive">Ko'p urinish. Biroz kuting.</span>
            </div>
          )}
          ...
          >
            {isSubmitting ? (
              ...
              <span>{cooldownSecs} soniyadan so'ng qayta urinib ko'ring</span>
            ) : (
              <span className="flex items-center gap-2">
