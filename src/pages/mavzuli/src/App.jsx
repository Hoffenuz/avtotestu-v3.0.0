// NOTE: This is the canonical App for the standalone mavzuli sub-application
// (src/pages/mavzuli/). It is built separately via its own vite.config.js and
// is NOT part of the main Vite build. The main app uses MavzuliTestlar.tsx +
// MavzuliTestInterface.tsx instead. Credentials must come from env variables only.
...
// Supabase client — module darajasida bir marta yaratiladi (har renderda emas)
const supabase = createClient(
  import.meta.env.VITE_SUPABASE_URL,
  import.meta.env.VITE_SUPABASE_ANON_KEY || import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY
);

// Bundle local JSON test files from source so they work without copying to public/
...
function App() {
  ...
  const [navHasLeft, setNavHasLeft] = useState(false);
  const [navHasRight, setNavHasRight] = useState(false);
  const [isFullscreen, setIsFullscreen] = useState(false);

  // When test starts or questions change, ensure navigation resets to the start
  useEffect(() => {
    if (isStarted && questionNavRef && questionNavRef.current) {
      try {
        questionNavRef.current.scrollTo({ left: 0, behavior: 'smooth' });
      } catch (e) {}
    }
  }, [isStarted, questions]);

  // update nav fade visibility and persist scroll position for current topic
  useEffect(() => {
    const el = questionNavRef.current;
    if (!el) return;

    const check = () => {
      setNavHasLeft(el.scrollLeft > 2);
      setNavHasRight(el.scrollWidth > el.clientWidth + el.scrollLeft + 2);
      try {
        const key = `navScroll_${selectedTopic || 'default'}`;
    ...
    try {
      ...
      if (saved && saved > 0) el.scrollTo({ left: saved, behavior: 'auto' });
    } catch (e) {}

    el.addEventListener('scroll', check, { passive: true });
    check();
    return () => el.removeEventListener('scroll', check);
  }, [questionNavRef, selectedTopic, questions.length]);

  // ensure current question is visible in nav on change
  useEffect(() => {
    const el = questionNavRef.current;
    if (!el) return;
    const btn = el.querySelectorAll('.question-number')[currentQuestionIndex];
    if (btn) {
      ...
      if (btnLeft < viewLeft || btnRight > viewRight) {
        const target = Math.max(0, btnLeft - (el.clientWidth / 4));
        el.scrollTo({ left: target, behavior: 'smooth' });
      }
    }
  }, [currentQuestionIndex]);

  // Fullscreen helpers (support toggling fullscreen for the test view)
    ...
    async function loadTopics() {
      try {
        const topics = [
          ...
          { id: '15', name: 'Yo\'lning qatnov qismida transport vositalarining joylashuvi', filename: '15.json' },
          ...
          { id: '32', name: 'Yangi Savollar' }, // faqat supabase (questions1/choices1 jadvallaridan)
        ...
        if (import.meta.env.DEV) console.error('Mavzularni yuklashda xatolik:', error);
        setLoading(false);
      }
    }
  ...
  const loadQuestionsFromSupabase = async () => {
    const { data: questions, error: qError } = await supabase
      ...
      .select('*');
    if (qError) {
      setErrorMessage('Supabase savollarini yuklashda xatolik!');
      throw qError;
    }
    const { data: choices, error: cError } = await supabase
      ...
      .select('*');
    if (cError) {
      setErrorMessage('Supabase variantlarini yuklashda xatolik!');
      throw cError;
    }
    // Data logs removed — never expose question/answer data in production
  ...
  // startTest and selectTopic are defined once later (after loaders) to pick source by topic id
  ...
  const loadQuestionsFromFile = async (filename) => {
    try {
      ...
      if (LOCAL_TEST_FILES && LOCAL_TEST_FILES[localKey]) {
        ...
        // Primary: root public/testlar where you placed files
      ...
      if (Array.isArray(data)) raw = data;
        ...
        // Sometimes the JSON is an object with numbered keys or sections
      ...
      const transformedQuestions = raw.map((item, index) => {
        ...
        // Helper to read question text from multiple shapes
        ...
        // Image detection across common fields
        const imgField = item.photo || item.image || item.img || item.photo_url || item.image_url;
        let image_url = null;
        // Images for this page are stored under public/mavzuli — prefer that folder for local names
        const MAVZULI_PUBLIC_PREFIX = '/mavzuli/';
        if (imgField) {
          const s = String(imgField).trim();
          if (/^https?:\/\//i.test(s)) {
            image_url = s;
          } else {
            // If the value already contains an image extension, use it but point into mavzuli folder
            if (/\.(png|jpg|jpeg|webp|gif)$/i.test(s)) {
              image_url = MAVZULI_PUBLIC_PREFIX + s.replace(/^\//, '');
            } else {
              // No extension — assume WEBP in the mavzuli public folder
              image_url = MAVZULI_PUBLIC_PREFIX + s.replace(/^\//, '') + '.webp';
            }
          }
        } else if (item.media && item.media.name) {
          const name = String(item.media.name).trim();
          if (/^https?:\/\//i.test(name)) {
            image_url = name;
          } else if (/\.(png|jpg|jpeg|webp|gif)$/i.test(name)) {
            image_url = MAVZULI_PUBLIC_PREFIX + name.replace(/^\//, '');
          } else {
            image_url = MAVZULI_PUBLIC_PREFIX + name.replace(/^\//, '') + '.webp';
          }
        }

        // Choices normalization: support answers.answer (localized), answers array, options/variants, legacy choises
        let choices = [];
      ...
      if (import.meta.env.DEV) console.error('Savollarni yuklashda xatolik:', error);
      throw error;
    }
  };
  ...
  const selectTopic = async (topic) => {
    ...
    try {
      ...
      // If topic is "Yangi Savollar" (id 32) load from Supabase, otherwise from JSON file
      const isNewQuestions = String(topic.id) === '32' || topic.name === 'Yangi Savollar';

      if (isNewQuestions) {
        // Load fresh questions from Supabase
        ...
        // No source defined for this topic
        throw new Error('Mavzu uchun manba topilmadi (filename mavjud emas).');
      }

      // Use all questions from the selected source
      ...
      // ensure start from first question and reset nav scroll
      setCurrentQuestionIndex(0);
      setTimeout(() => {
        if (questionNavRef && questionNavRef.current) {
          try { questionNavRef.current.scrollTo({ left: 0, behavior: 'smooth' }); } catch (e) {}
        }
      }, 60);

      setLoading(false);
    } catch (error) {
      if (import.meta.env.DEV) console.error('Testni boshlashda xatolik:', error);
      setErrorMessage(error.message || 'Testni boshlashda xatolik');
      setLoading(false);
    }
  };
            ...
            {question.image_url && (
              <div className="question-image" key={question.id}>
                <img
                  ...
                  alt="Savol rasmi"
                  onError={(e) => { e.target.src = DEFAULT_QUESTION_IMAGE; }}
                  loading="eager"
                />
              </div>
            )}
  ...
  const renderWelcomeScreen = () => (
    <div className="welcome-screen">
      ...
      <div className="test-type-buttons">
        ...
        <a href="/" className="start-button">Bosh saxifa</a>
    ...
    <div className="landing-screen">
      <div className="landing-top" style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <div style={{display:'flex', alignItems:'center', gap:12}}>
          <button className="home-button" onClick={() => { window.location.href = '/'; }}>Bosh sahifa</button>
          <div className="language-selector">
            {SUPPORTED_LANGS.map(l => (
              <button
                key={l.code}
        ...
        {/* keep right side empty to allow left-aligned controls */}
        <div />
      </div>
    ...
    <>
      <div className="header-controls">
        ...
        <div className="header-buttons">
          ...
          <button
            className="finish-button-header"
            title={isFullscreen ? 'Fullscreendan chiqish' : 'Fullscreen qilish'}
            onClick={() => {
      ...
      <div style={{ position: 'relative' }}>
        {/* Mobile nav arrows (visible only on small screens via CSS) */}
